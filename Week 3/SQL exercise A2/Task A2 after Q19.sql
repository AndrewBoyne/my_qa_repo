...
month (Date) as Month
, max (Max_Wind_Speed_MPH) as 'Maximum wind speed'
from Seattle_weather_conditions
where year (Date) = 2015
group by month (Date)
order by 'Maximum wind speed'desc
...
, Events
from Seattle_weather_conditions
where not events = 'rain' --the answer is 41 (from looking down the table) but I need to be able to count or sum 
--(in case the table has thousands of rows)

--the answer is to disregard the date.  I only need to know the number of times the event is not just rain so ->
...
and month (Date) like '1%' and month (Date) like '2%' and month (Date) like '3%'
--group by Year (Date)

select 
sum(Precipitation_In) as 'Accumulated Rainfall in Jan, Feb, Mar 2016'
from Seattle_weather_conditions
where month (date) in(1,2,3)
and year (date) = 2016
--order by Precipitation_In
